...
class ShatteringWidget extends StatefulWidget {
  ...
  const ShatteringWidget({
    ...
    required this.onShatterCompleted,
  }) : super(key: key);
...
class Tri {
  ...
  Offset get center => Offset((p1.dx + p2.dx + p3.dx) / 3.0, (p1.dy + p2.dy + p3.dy) / 3.0);
}
...
class Shard {
  ...
  Offset getCenter(Size size) {
    return Offset((triangle.p1.dx * size.width + triangle.p2.dx * size.width + triangle.p3.dx * size.width) / 3.0,
        (triangle.p1.dy * size.height + triangle.p2.dy * size.height + triangle.p3.dy * size.height) / 3.0);
  }
...
class ShatteringWidgetState extends State<ShatteringWidget> with SingleTickerProviderStateMixin {
  Random r = Random();
  GlobalKey key = GlobalKey();
  late AnimationController controller;
  late ui.Image image;
  late List<Shard> shards;
    ...
    boundary.toImage().then((value) {
      setState(() {
        ...
        shards = triangles
            ...
            // .map((e) => r.nextBool() ? e.split() : [e])
            // .expand((e) => e)
            .map((e) => Shard(e, -.3 + r.nextDouble() * .6, ((e.center - Offset(.5, .5)) * r.nextDouble() * 600)))
            .toList();
        controller.forward();
      });
    });
...
class ShatteringWidgetPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (Shard shard in shards) {
      ...
      canvas.translate(
          -center.dx + shard.velocity.dx * shatterProgress, -center.dy + shard.velocity.dy * shatterProgress);
